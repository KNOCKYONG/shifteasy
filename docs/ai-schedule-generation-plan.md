...
- 커버리지 솔버는 동일 팀 구성원이 같은 시프트 슬롯을 연속으로 채우면 비용을 기하급수로 늘려, 요구 인원이 2명 이상인 시프트는 가능한 경우 자동으로 2개 이상의 팀이 섞이도록 한다. 팀 정보가 없는 인원은 페널티에서 제외해 기존 운영팀이 부족하더라도 배정이 막히지 않게 한다.
...
- `D/E/N` 기본 시프트 외에 사용자가 직접 정의한 근무 타입은 부서 패턴이 따로 없으면 순환 제약을 적용하지 않고, 직원별 누적 배정 횟수에 따라 페널티를 주어 구성원 간에 고르게 분배한다.
- 부서 패턴 또는 시프트 템플릿에 명시적인 값이 없더라도 `D/E/N` 시프트는 자동으로 최소 1명의 필수 인원을 갖도록 기본값을 주입해, 어느 날이든 세 시프트가 모두 비어 있는 상태가 발생하지 않도록 한다.
...
- 1차 패스로도 필수 인원을 채우지 못하면 “OFF 한도 오버라이드” 모드로 동일 탐색기를 한 번 더 실행한다. 이 모드는 월간 휴무 한도를 이미 채운 직원도 후보에 포함시키되, 점수에 큰 페널티(`OFF_OVERRIDE_PENALTY`)를 부여해 다른 후보가 있으면 항상 일반 모드가 우선 선택되게 한다.
