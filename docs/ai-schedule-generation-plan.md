...
- 실행될 때마다 `randomSeed`를 새로 생성해 동일한 입력이더라도 결과가 다르게 나올 수 있도록 한다. (기본 시드 3개 + 필요 시 폴백 2개를 난수로 뽑아 최적 조합을 선택)
...
- 3교대 인원이 이미 월간 휴무 한도(`maxOffDays`)를 채운 상태라면, 해당 날짜에 요구 인원을 모두 채웠더라도 D/E/N 중 하나를 **강제로 과충원(over-allocation)** 하여 지원 근무로 전환한다. 이렇게 해서라도 OFF 초과를 막고, 부득이하게 추가 OFF를 부여한 경우에만 `extraOffDays`로 보고한다.
...
- 시프트별 필수 팀 수(`teamTargets`)를 `min(필수 인원, 해당 시프트 배치 가능한 팀 수)`로 계산하고, 목표 이하일 때는 다른 팀을 우선 배정하도록 큰 보너스/페널티를 걸어 “팀별 최소 1명 이상” 기준을 강하게 유지한다.
- `workPatternType === 'night-intensive'` 직원은 어떤 상황에서도 야간(`N`) 외 시프트로 전환되지 않는다. 지원 근무/오버라이드 시나리오에서도 `N` 배정만 허용하고, 야간 슬롯이 없으면 OFF로 유지한다.
...
- 오버라이드 모드에서는 회복 OFF, 회전 락, 야간→주간 금지 등의 필터도 강등해 강한 페널티(`CRITICAL_OVERRIDE_PENALTY`)를 부여한 채 후보에 포함시킨다. 즉, 커버리지 미달 상황에서는 인력 제약보다 “필수 인원 충족/팀 분산”이 우선순위가 된다.
- 이 단계에서 결정된 배정은 `updateEmployeeState`/`incrementOffCounter`를 통해 기존 상태 머신과 동일하게 누적되므로, 이후 휴무/회복 규칙과 자연스럽게 이어진다.
- 같은 입력으로도 더 좋은 조합을 찾기 위해 서로 다른 random seed(기본/13/29/47 등)로 여러 번 커버리지 탐색을 돌리고, 각 결과를 즉시 `validateSchedule`로 평가해 가장 점수가 높은 후보를 택한다.
...
- 락된 날은 직전 패스의 배정을 그대로 재사용하고, 나머지 날은 CP-SAT 솔버를 다시 실행해 부분 재배정을 수행한다. 이 과정을 최대 5회까지 반복해, 개선이 없더라도 보강 대상이 남아 있는 동안은 계속 탐색한다.
- 윈도우 선택 기준: `affectedDates`가 있는 violation 우선 → 없으면 일정 길이의 연속 블록을 순차적으로 순회한다. 덕분에 특정 구간만 파괴/재구축할 수 있어 성능 부담 없이 품질을 개선한다.

### 2.7 자동 보강 루프
- LNS 이후 완성된 스케줄을 다시 분석해 날짜·시프트별 필수 인원/팀 목표(`teamTargets`)를 만족하지 못한 지점을 수집하고, 동일 날짜를 `destroyedDates`로 지정해 부분 재생성을 다시 수행한다.
- 각 보강 패스는 `validateSchedule`의 `coverage` 위반과 자체 분석 결과를 결합해 대상 날짜를 판정하며, 해당 날짜는 `allowOffOverride`가 항상 켜진 상태에서 탐색해 휴무 한도보다 커버리지가 우선되도록 한다.
- 반복 횟수는 최대 3회로 제한하고, 새 패스가 기존 대비 위반 수를 줄이거나 점수를 높일 때만 채택한다. 개선이 없으면 즉시 중단하고 직전 최선의 결과를 반환한다.
...
- **지원 근무/휴무 분배 개선**: 지원 근무는 “필수 인원 부족 + 팀 다양성에 기여”하는 시프트부터 채우고, OFF lag(월간 목표 대비 2일 이상 지연)이 발생한 직원은 강제로 OFF 후보군에 넣어 월말 휴무 몰림을 막는다.
- **After-build 휴리스틱**: 기본 탐색이 끝나면 요청이 어긋난 직원과 동일 날짜·시프트의 다른 직원을 자동 스왑해 잔여 위반을 줄이고, 결과를 다시 검증해 반영한다.
- **시뮬레이션 기반 스코어링**: Seed별 파이프라인 결과의 점수를 비교해 최고안을 선택하고, 목표 점수(`MIN_SCORE_TARGET`)에 미달하면 추가 Seed(예: 97/131)로 전체 파이프라인을 재실행한다. 충분한 점수에 도달할 때까지 반복하므로 요구 정확도가 보장된다.
- **향후 고정밀 옵션**: 3~5일 블록 단위 ILP·사후 스왑 휴리스틱·시뮬레이션 기반 Global cost loop 등을 추가해 “느리지만 매우 정확한 모드”를 제공할 계획이다. 요구 조건이 매우 엄격한 테넌트는 해당 모드를 선택해 더 높은 만족도를 얻을 수 있다.
