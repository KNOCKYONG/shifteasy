...
- 기본 순환은 `D → (O) → E → (O) → N` 순으로 가중치를 부여하며, OFF가 사이에 껴 있어도 직전 실제 근무 시프트를 기준으로 다음 시프트의 보너스/페널티가 계산된다. 특히 `D` 이후에는 `E`를, `E` 이후에는 `N`을, `N` 이후에는 우선 `O`, 불가 시 `E`를 권장하되 나이트 집중 근무자는 예외로 두어 연속 `N`을 최대한 유지한다.
