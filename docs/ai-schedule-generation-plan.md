...
- `workPatternType === 'night-intensive'` 직원은 원칙적으로 야간(`N`) 시프트에만 배정하며, 다른 시프트가 필요한 경우에는 남은 `nightIntensivePaidLeaveDays`(유급 휴가 일수)를 페널티로 소비하면서만 예외를 허용한다. 유급 휴가 잔여분이 0이면 오버라이드가 불가하고, 잔여분을 사용할수록 강한 비용이 붙어 사실상 N 고정이 유지된다.
